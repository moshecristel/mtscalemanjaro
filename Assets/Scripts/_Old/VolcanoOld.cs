...
public class VolcanoOld : MonoBehaviour
{
    [SerializeField] private VolcanoBlockOld volcanoBlockOldPrefab;
    ...
    void Awake()
    {
        // BoxCollider blockCollider = volcanoBlockOldPrefab.GetComponent<BoxCollider>();
            ...
            for (int xOffsetNormalized = -currRadius; xOffsetNormalized <= currRadius; xOffsetNormalized++)
            {
                for (int zOffsetNormalized = -currRadius; zOffsetNormalized <= currRadius; zOffsetNormalized++)
                {
                    ...
                    if (normalizedDistance <= currRadius)
                    {
                        ...
                        VolcanoBlockOld blockOld = Instantiate(volcanoBlockOldPrefab, transform);
                        blockOld.transform.position = new Vector3(xOffset, i * _blockDimension, zOffset);
                        blockOld.LevelIndex = i;
                        blockOld.OriginNormalizedOffset = new Vector3Int(xOffsetNormalized, i, zOffsetNormalized);
                        currLevel.GroundLevelOffsetToBlock[
                            new Vector3Int(xOffsetNormalized, (int)groundCenter.y, zOffsetNormalized)] = blockOld;
                    }
                }
            }
    ...
    public void ApplyNoise(bool isSubtract = true)
    {
        ...
        List<VolcanoBlockOld> candidateBlocks = new();
        
        foreach (VolcanoBlockOld block in levelToApplyNoiseTo.GroundLevelOffsetToBlock.Values)
        {
        ...
        VolcanoBlockOld selectedBlockOld = candidateBlocks[Random.Range(0, candidateBlocks.Count)];
        if (isSubtract)
        {
            levelToApplyNoiseTo.GroundLevelOffsetToBlock.Remove(new Vector3Int(selectedBlockOld.OriginNormalizedOffset.x,
                0, selectedBlockOld.OriginNormalizedOffset.z));
            Destroy(selectedBlockOld.gameObject);
